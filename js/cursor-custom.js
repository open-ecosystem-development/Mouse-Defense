...
export class CursorCustom extends Component {
    ...
    static Properties = {
        ...
        /** Object that contains mesh component`. */
        cursorRayMesh: { type: Type.Object },
        /** Axis along which to scale the `cursorRayObject`. */
        cursorRayScalingAxis: { type: Type.Enum, values: ['x', 'y', 'z', 'none'], default: 'z' },
    ...
    init() {
        ...
        onSceneLoaded.push(sceneLoaded);
        this.onDestroyCallbacks = [() => {
            const index = onSceneLoaded.indexOf(sceneLoaded);
            if (index >= 0) onSceneLoaded.splice(index, 1);
        }];
    }
    start() {
        ...
        onXRSessionStart.push(onXRSessionStart);
        this.onDestroyCallbacks.push(() => {
            const index = onXRSessionStart.indexOf(onXRSessionStart);
            if (index >= 0) onXRSessionStart.splice(index, 1);
        });
    ...
    _setCursorVisibility(visible) {
        ...
        let thisMesh = this.cursorRayMesh.getComponent('mesh');
        if (thisMesh) {
            thisMesh.active = this.visible;
        }
        if (visible) {
    ...
    doUpdate(doClick) {
        ...
        if (this.session) {
            /* Since Google Cardboard tap is registered as arTouchDown without a gamepad, 
            we need to check for gamepad presence */
            if (
                this.arTouchDown && this.input &&
                xrSession.inputSources[0].handedness === 'none' &&
                xrSession.inputSources[0].gamepad
            ) {
                const p = xrSession.inputSources[0].gamepad.axes;
            ...
            const rayHit = this.rayHit = (this.rayCastMode == 0) ?
                scene.rayCast(this.origin, this.direction, this.collisionMask) :
                physics.rayCast(
                    this.origin,
                    this.direction,
                    this.collisionMask,
    ...
    hoverBehaviour(rayHit, doClick) {
        if (rayHit.hitCount > 0) {
            if (!this.hoveringObject || !this.hoveringObject.equals(rayHit.objects[0])) {
                ...
                if (this.styleCursor) canvas.style.cursor = "pointer";
            ...
            if (this.styleCursor) canvas.style.cursor = "default";
        }
    ...
    updateDirection() {
        ...
        const rayHit = this.rayHit = (this.rayCastMode == 0) ?
            scene.rayCast(this.origin, this.direction, this.collisionMask) :
            physics.rayCast(this.origin, this.direction, this.collisionMask, this.maxDistance);
